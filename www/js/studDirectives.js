studApp.controller('$ionicFilterBarModalCtrl', [
      '$window',
      '$scope',
      '$timeout',
      '$ionicListDelegate',
      function ($window, $scope, $timeout, $ionicListDelegate) {
        ...
        console.log("coming from search Filter with text in ionic controller..", searchText);

        // $scope.orgs = resultOrgs;
        // $scope.$apply();

        });

        $scope.$on('searchFilterGrp', function(event, searchText) {
        console.log("coming from search Filter Group with text in ionic controller..", searchText);
  ...
  studApp.directive('ionFilterBar', [
    '$timeout',
    '$ionicGesture',
      ...
      if ($ionicPlatform.is('android')) {
        filterBarTemplate =
          '<div class="filter-bar-wrapper filter-bar-{{::config.theme}} filter-bar-transition-{{::config.transition}}">' +
            '<div  class="bar bar-header bar-{{::config.theme}} item-input-inset" style="background-color: #9ACD32;">' +
              '<button  class="filter-bar-cancel button button-icon icon {{::config.back}}" ng-click="onBack()"></button>' +
              '<label class="item-input-wrapper" style="background-color: #9ACD32;">' +
                '<input type="search" class="filter-bar-search"  ng-model="data.filterText" placeholder="Search here.." ng-keypress="justEnt($event)"/>' +
          ...
          '<div class="filter-bar-wrapper filter-bar-{{::config.theme}} filter-bar-transition-{{::config.transition}}">' +
            '<div class="bar bar-header bar-{{::config.theme}} item-input-inset">' +
              '<label class="item-input-wrapper">' +
                ...
                '<input type="search" class="filter-bar-search" ng-model="data.filterText" placeholder="nodana4"/>' +
                '<button class="filter-bar-clear button button-icon icon" ng-class="getClearButtonClass()"></button>' +
              '</label>' +
      ...
      return {
        ...
        link: function ($scope, $element) {
          ...
          $scope.onBack = function() {
            console.log("coming for the directive function");
            rootScope.pCounter = 0;
            rootScope.$broadcast('onBack');
            rootScope.$broadcast('onBackGrp');
            rootScope.$broadcast('onBackOrgPer');
            rootScope.$broadcast('onBackPer');
            rootScope.$broadcast('onBackCont');
          }

          $scope.justEnt = function(keyEvent) {
              if (keyEvent.which === 13)
                console.log("got it in main...");
                console.log("The values is...", $scope.data.filterText);
                rootScope.sCounter = 0;
                rootScope.$broadcast('searchFilter', $scope.data.filterText);
                rootScope.$broadcast('searchFilterGrp', $scope.data.filterText);
                rootScope.$broadcast('searchFilterOrgPer', $scope.data.filterText);
                rootScope.$broadcast('searchFilterPer', $scope.data.filterText);
                rootScope.$broadcast('searchFilterCont', $scope.data.filterText);
              }

          rootScope.$on('cancelfilter', function(event) {

                console.log("coming to cancel the filter..");
 ...
 studApp.provider('$ionicFilterBarConfig', function () {
...
studApp.factory('$ionicFilterBar', [
      '$document',
      '$rootScope',
      '$compile',
      '$timeout',
      ...
      function ($document, $rootScope, $compile, $timeout, $filter, $ionicPlatform, $ionicFilterBarConfig, $ionicConfig, $ionicModal, $ionicScrollDelegate) {
        ...
        function filterBar (opts) {
          ...
          // scope.hideFilterBar(scope.done);
          // scope.removeFilterBar(scope.done);
